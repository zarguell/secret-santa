...
export class Party extends DurableObject {
  ...
  async createParty(
    data: Omit<PartyData, "assignments" | "guestLinks" | "createdAt">,
  ): Promise<{
    partyId: string;
    ...
    return {
      ...
      guestUrls: Object.fromEntries(
        Object.entries(guestLinks).map(([guest, id]) => [
          guest,
          `/guest/${id}`,
        ]),
      ),
  ...
  async assignGift(req: AssignRequest): Promise<AssignmentResponse> {
    ...
    if (
      !partyData.assignments ||
      Object.keys(partyData.assignments).length === 0
    ) {
